...
# Graf 1: Broj grešaka po tipu
ax1.bar(['Insertions', 'Substitutions'], [insertions, substitutions], color=['blue', 'red'])
ax1.set_title('Broj grešaka')
ax1.set_ylabel('Broj')

# Graf 2: Top 10 zamjena (ako postoje)
if len(char_subs) > 1 and char_subs['count'].sum() > 0:
    ...
    ax2.set_title('Najčešće zamjene')
else:
    ax2.text(0.5, 0.5, 'Nema zamjena\n(0 substitutions)', ha='center', va='center', transform=ax2.transAxes)
    ax2.set_title('Zamjene')

plt.tight_layout()
plt.savefig('grafovi_rezultata.png', dpi=300)
plt.show()
